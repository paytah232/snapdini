// Generate the social-share (Open Graph) image for Snapdini → web/static/og.png.
// Lives under src/ so it's visible inside the app container (only ./src is bind-mounted).
// Needs sharp + fonts (fontconfig + ttf-dejavu, baked into the dev image). To regenerate:
//
//   docker compose -f docker-compose.dev.yml exec app node src/scripts/make-og.mjs /app/src/og.png
//   cp app/src/og.png web/static/og.png
//   docker compose -f docker-compose.dev.yml exec -u root app rm /app/src/og.png
//   # rebuild web so the new asset is served:
